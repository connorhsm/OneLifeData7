...
def list_dir(folderpath=".", file=False, folder=False, silent=True):
    ...
    for filename in os.listdir(folderpath):
        ...
        if not file and os.path.isfile(fullpath):
            continue
        if not folder and os.path.isdir(fullpath):
            continue
...
changes_deleted = [
    e.replace("\\", "") for e in os.environ.get("CHANGES_DELETED").split("\n")
]
changes_added = [
    e.replace("\\", "") for e in os.environ.get("CHANGES_ADDED").split("\n")
]
    ...
    if "objects/" in changed_file or "categories/" in changed_file:
        ...
        if object_id != -9999:
            ...
            if "objects/" in changed_file:
                object_lines.append(
                    f"{sign} [{object_id}](https://github.com/{repo}/pull/{pr_number}/files#diff-{file_change_hash}) {object_name}"
                )
        ...
        for i in range(2, len(transition_file_content_parts)):
            trans_keyValuePairs.append(
                (fields[i + 3], transition_file_content_parts[i])
            )
        ...
        transition_details = "\r\n".join(
            [f"{e[0]}: {e[1]}" for e in trans_keyValuePairs]
        )
